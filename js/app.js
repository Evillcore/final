...
if (filtersItems.length > 0) {
	let filters = document.querySelectorAll('.works__menu');
	if (filters.length > 0) {
		for (let index = 0; index < filters.length; index++) {
			const filter = filters[index];
			filter.addEventListener("click", function (e) {
				const filterActive = document.querySelector('.works__menu._active');
				const filterValue = filter.dataset.filter;
				filterActive.classList.remove('_active');
				filter.classList.add('_active');


				showBlogItems(filterValue);

			});
		}

		function showBlogItems(filter) {
			for (let index = 0; index < filtersItems.length; index++) {
				const filtersItem = filtersItems[index];
				filtersItem.classList.add('_hide');
				if (filter === 'all' || !filter) {
					filtersItem.classList.remove('_hide');
				} else if (filtersItem.classList.contains('works__item_' + filter)) {
					filtersItem.classList.remove('_hide');
...
function scroll_scroll() {
	...
	let spanScroll = document.querySelector('.intro__scroll');
	if (header !== null) {
		if (src_value > 50) {
	...
	if (spanScroll !== null) {
		if (src_value > 10) {
			...
			spanScroll.classList.add('_scroll');
		} /* else {
			spanScroll.classList.remove('_scroll');
		} */
	}
